...
(function () {
    ...
    w2grid.prototype = {
        ...
        msgRefresh      : w2utils.lang('Обновление...'),
        ...
        buttons: {
            ...
            'columns'  : { type: 'drop', id: 'w2ui-column-on-off', icon: 'w2ui-icon-columns', hint: w2utils.lang('Показать/спрятать колонку'), arrow: false, html: '' },
            'search'   : { type: 'html',   id: 'w2ui-search',
                            html: '<div class="w2ui-icon icon-search-down w2ui-search-down" title="'+ 'Выбрать поля для поиска' +'" '+
                                  'onclick="var obj = w2ui[$(this).parents(\'div.w2ui-grid\').attr(\'name\')]; obj.searchShowFields();"></div>'
                          },
            'search-go': { type: 'check',  id: 'w2ui-search-advanced', caption: w2utils.lang('Поиск...'), hint: w2utils.lang('Open Search Fields') },
            'add'      : { type: 'button', id: 'w2ui-add', caption: w2utils.lang('Добавить'), hint: w2utils.lang('Добавить новую запись'), icon: 'w2ui-icon-plus' },
            'edit'     : { type: 'button', id: 'w2ui-edit', caption: w2utils.lang('Edit'), hint: w2utils.lang('Редактировать выделенные записи'), icon: 'w2ui-icon-pencil', disabled: true },
            'delete'   : { type: 'button', id: 'w2ui-delete', caption: w2utils.lang('Удалить'), hint: w2utils.lang('Удалить выбранные записи'), icon: 'w2ui-icon-cross', disabled: true },
            'save'     : { type: 'button', id: 'w2ui-save', caption: w2utils.lang('Сохранить'), hint: w2utils.lang('Сохранить измененые записи'), icon: 'w2ui-icon-check' }
        },
        ...
        searchShowFields: function () {
            ...
            for (var s = -1; s < this.searches.length; s++) {
                ...
                if (s == -1) {
                    ...
                    search = { field: 'all', caption: w2utils.lang('Все поля') };
                } else {
        ...
        initColumnOnOff: function () {
            ...
            col_html += '<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].stateSave(); $(\'#w2ui-overlay\')[0].hide();">'+
                        '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Запомнить настройки') + '</div>'+
                        '</td></tr>'+
                        '<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].stateReset(); $(\'#w2ui-overlay\')[0].hide();">'+
                        '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Востановить настройки') + '</div>'+
                        '</td></tr>';
            col_html += "</table></div>";
        ...
        scroll: function (event) {
            ...
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            $('#grid_'+ this.name + '_footer .w2ui-footer-right').html(w2utils.formatNumber(this.offset + t1) + '-' + w2utils.formatNumber(this.offset + t2) + ' ' + w2utils.lang('из') + ' ' +    w2utils.formatNumber(this.total) +
        ...
        status: function (msg) {
            if (typeof msg != 'undefined') {
                ...
                if (sel.length > 0) {
                    ...
                    if (sel.length == 1) msgLeft = w2utils.lang('Запись ID') + ': '+ tmp + ' ';
                }
...
var w2alert = function (msg, title, callBack) {
    if (title == null) title = w2utils.lang('Оповещение');
...
(function () {
    var w2form = function(options) {
        ...
        this.msgRefresh    = w2utils.lang('Обновление...');
        this.msgSaving     = w2utils.lang('Сохранение...');
        ...
        validate: function (showErrors) {
            ...
            for (var f in this.fields) {
                ...
                if (field.required && (val === '' || ($.isArray(val) && val.length == 0) || ($.isPlainObject(val) && $.isEmptyObject(val)))) {
                    errors.push({ field: field, error: w2utils.lang('Обязательное поле') });
                }
