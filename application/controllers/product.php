...
function item($id){
	...
	$this->load->view('product_item' ,$data);
...
function editProduct(){
		...
		$this->load->library('upload', $config);
	 	...
	 	$this->upload->do_upload('gabchertej');
		$upload_data = $this->upload->data();
	//	echo$upload_data['file_name'];die;
		 echo $data['gab_chertej'] = $upload_data['file_name'];

		 if ($data['gab_chertej']=="") {
			$data['gab_chertej']=$_POST['hiddengabchertej'];
		}
		...
		/*загрузка фотографий товара для 360*/
			...
			$config['upload_path'] = './uploads/products/productid'.$ID;
			$config['allowed_types'] = 'jpg|png';
			$config['max_size']	= '0';
			$config['max_width'] = '0';
			$config['max_height'] = '0';
